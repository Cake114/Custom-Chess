...
// 通用目录扫描函数：读取 GitHub Actions 自动生成的 filelist.json 配置文件
async function scanDirectory(path, extension) {
    try {
        // 请求本地静态的 filelist.json 资源清单，加入随机参数强刷缓存
        const response = await fetch(`./filelist.json?t=${Date.now()}`);
        if (!response.ok) {
            console.warn(`[扫描跳过] 无法读取 filelist.json 清单文件`);
            return [];
        }
        const data = await response.json();
        
        // 格式化 path 键值，剔除末尾斜杠，例如 "buff/" 转化为 "buff"
        const folderKey = path.replace(/\/$/, '');
        const files = data[folderKey] || [];

        // 过滤出指定后缀的文件名并拆除后缀，保持与游戏原本 scanDirectory 返回结果高度一致
        return files
            .filter(fileName => fileName.endsWith(extension))
            .map(fileName => fileName.replace(extension, ''));
    } catch (e) {
        console.warn(`扫描清单目录 ${path} 失败:`, e);
        return [];
    }
}
...
function updateHPDisplay() {
    ...
    // 更新按钮 status：非我方回合时禁用并暗色处理，保持布局稳定
    // 预检技能释放条件，用于控制确定按钮状态
